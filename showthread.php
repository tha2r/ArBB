...
$phrasearray = array('poll','forumdisplay','showthread','showteam','profile','editor');



require 'global.php';

$pluginlist = 'showthread_start,showthread_poll_start,showthread_polloption,showthread_results,showthread_post_start'
             .',showthread_postbit_create,showthread_loggedinuser,showthread_complete';
...
$thread_query=$DB->query("select t.*,f.title as forumtitle,f.mainid,f.parentlist,f.forumid from "._PREFIX_."thread t LEFT JOIN "._PREFIX_."forum f on(f.forumid=t.forumid) where t.threadid='$tid'");
        ...
        $local['whereurl']='showthread.php?tid='.$tid;
        ...
        if($thread['pollid']>0)
        {
           ...
           {
               $showresult=0;
              $plugins->load('showthread_poll_start');
               if($poll['timeout']>0)
               {
               ...
               if(!$showresult)
               {
                   ...
                   while(isset($opt[$i])&&(strlen($opt[$i])>0))
                   {
                    ...
                    $i++;
                     ...
                     $plugins->load('showthread_polloption');
                    $TP->WebTemp('poll_options_table');
                       ...
                       else
                       {
                                           ...
                                           while(isset($opt[$i])&&(strlen($opt[$i])>0))
                                           {
                                            ...
                                            $plugins->load('showthread_results');
...
//# Continue for displaying thread
                   ...
                   {
                   $plugins->load('showthread_post_start');
                   ...
                   $plugins->load('showthread_postbit_create');
                   $postsbit.=$TP->GetTemp('showthread_postbit');
                   }
         ...
         $act['users']=0;
             ...
             if($act['num']>0)
             {
                    ...
                    {
                     $plugins->load('showthread_loggedinuser');
                     if($online[userid]>0)
                     {
                            $act['users']++;
      ...
      $quickreply=$TP->GetTemp('showthread_quickreply');
   ...
   $plugins->load('showthread_complete');
